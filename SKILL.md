...
# 공공기관 보고서 작성 + HWPX 변환 스킬 (v3.6.12)
...
#### 4-1. 자동 적용 (신뢰도 높음)
- `~와 관련된` → `~ 관련`
- `~할 예정이었으나 이를 유예하였습니다` → `~ 예정 → 유예`  *(명사형 종결 — 1p·풀버전만, 아래 장르 가드 참조)*
- `여러/많은/각/모든/수많은/대부분의/다양한 ~들` → 동일 단어에서 `들` 제거

#### 4-2. 검토 권장 표시 (사용자 확인용)
- `~할 것으로 보입니다` → `~ 예상 / ~ 전망`  *(명사형 종결 — 1p·풀버전만)*
- `~한 것으로 판단됩니다` → `~ 판단 / ~로 보임`  *(명사형 종결 — 1p·풀버전만)*
- `~하는 것이 필요합니다` → `~가 필요합니다`
- `~에 대한`, `~ 중 하나인`, `~의 ~의 ~` (의 연쇄)
- `사회적/경제적/정치적/행정적/조직적/기술적/사업적/업무적/제도적` + 명사
- 한 문장 46자 초과 (분리 후보)

> ⚠️ **장르 가드**: 서술어 종결을 **명사형(개조식)으로 바꾸는 변환**
> (`~보입니다`→`예상`, `~판단됩니다`→`판단`, `~예정이었으나 유예`→`예정→유예`)은
> **1페이지 보고서·풀버전 보고서에서만** 발동한다.
> **시행문·이메일에는 미발동** — 서술어 완결 + 공손체(`~하시기 바랍니다`)가 규범이므로
> 명사형 종결로 바꾸면 격식 위반. 시행문·이메일에서 문장이 길면 명사형 압축 대신
> **문장 분리**로 처리한다. (상세: `references/layout-rules.md` §8-1)

#### 4-3. 페이지 걸침 점검
- 양식별 페이지당 라인 수 추정 (`format_1p` ≈ 37줄 / 여백 상25·하20, `format_full` ≈ 38–42줄 / 여백 상30·하25)
...
- 분량: 1쪽 강제 (37줄 이내 — 1p 여백 상25·하20 기준 실효 상한)
...
3. **한 문장 한 핵심** — 한 문장에 두 개 정보 = 분리. 문장 길이는 3구간(≤35 안전 / 35–45 검토 / 46자↑ 강제분리, `layout-rules.md` §1).
...
| 24 | **자동변환 장르 가드** (v3.6.12) — 서술어 종결을 명사형(개조식)으로 바꾸는 변환(`~보입니다`→`예상`, `~판단됩니다`→`판단`, `~예정이었으나 유예`→`예정→유예`)은 **1p·풀버전에서만** 발동. **시행문·이메일에는 미발동** (서술어 완결+공손체 `~하시기 바랍니다` 가 규범). 시행문·이메일에서 긴 문장은 명사형 압축 대신 문장 분리로 처리. (상세: `references/layout-rules.md` §8-1) | 시행문·이메일에 개조식 문체 오적용 → 격식 위반 |

위 1–6은 v2~v3.0.1 의 규칙 그대로, 7–8은 v3.0 추가, 9–10은 v3.1 추가, 11은 v3.2 신규 규칙, 12–15는 v3.5.0 신규 규칙, 16은 v3.6.0 신규 규칙, 17은 v3.6.1 신규 규칙, 18은 v3.6.3 신규 규칙, 19는 v3.6.5 신규 규칙, 20은 v3.6.6/3.6.10 신규 규칙, 21은 v3.6.7~3.6.9 신규 규칙, 22는 v3.6.10 신규 규칙, 23은 v3.6.11 신규 규칙, 24는 v3.6.12 신규 규칙.
...
| 2026-05-14 | **3.6.12** | **작성원칙 정합성 핫픽스 (외부 리포분석 핸드오프 반영)** — 문서지능 프로젝트가 리포를 온톨로지 소스로 분석해 넘긴 수정사항 5건 반영. **(B) 자동변환 장르 가드** — 서술어→명사형 종결 변환(#3·4·5)이 양식 무관 발동하던 것을 1p·풀버전 전용으로 제한. 시행문·이메일(서술어 완결+공손체 규범)에 개조식 문체가 오적용되던 격식 위반 차단. `layout-rules.md` §8-1 신설 + Critical Rule 24 추가. **(C) 문장 길이 임계값 3구간 정본화** — writing-principles(35–45 vs 40)·SKILL(40 vs 46) 내부 불일치를, layout-rules 3구간 모델(≤35 안전 / 35–45 검토 / 46자↑ 강제분리)로 통일. **(D) 1p 라인수 게이트 정정** — 38줄 게이트가 풀버전 여백(상30·하25)에서 산출된 값이라 1p(상25·하20)에 과대 허용이던 것을 실효 상한 37줄로 하향 + 슬롯 기반 게이트를 더 정확한 대안으로 명시. **(E) 죽은 모듈 참조 정리** — v3.4.0 에서 제거된 `hwpx_helpers.py`/`layout_optimizer.py` 호출 패턴 서술을 5개 reference 파일(format-1p/full/gongmun/selection/email)에서 현행 슬롯 워크플로우로 교체. **(F) 날짜 범위 표기 통일** — `2025.01.05 ~ 2025.03.31`(0패딩) → `2025. 1. 5. ~ 2025. 3. 31.`(단일 날짜 규범 준수, 행안부 편람). **미반영(A)**: 1p 제목 하단바 이슈는 검증 결과 오진(그라데이션 존재, standard·skeleton 동일)이라 현행 유지. |
