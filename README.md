...
| 2026-05-14 | **3.6.12** | **작성원칙 정합성 핫픽스** — 자동변환 장르 가드(시행문·이메일에 개조식 문체 오적용 차단), 문장 길이 임계값 3구간 통일, 1p 라인수 게이트 37줄 정정, 죽은 모듈 참조 정리, 날짜 범위 표기 통일. E2E 검증(1p·시행문·풀버전 실빌드 + 산출물 문체 분기 측정)으로 빌드 명령 CLI 표기 오류 정정 (상세: [CHANGES.md](CHANGES.md)) |
