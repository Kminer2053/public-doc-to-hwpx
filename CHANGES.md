...
**E2E 검증 (2026-07-23)**: 장황한 AI 초안 1개를 1p·시행문 두 트랙으로 정제→실빌드→
산출물 텍스트 추출→객관 지표 측정. 13개 게이트 전건 통과. 핵심 증거 — 같은 사실이
양식별로 분기: 1p `증가 전망`(명사형) vs 시행문 `증가할 것으로 예상됩니다`(서술어 완결),
시행문 공손체(`감사드립니다`·`바랍니다`) 보존. 검증 과정에서 E 수정의 빌드 명령 CLI 표기
오류(`fill_skeleton.py --format` → 실제는 `--skeleton <경로>`, `expand_gongmun_body.py`는
`fill_skeleton.py`에 통합)를 발견·정정, 1p·시행문·풀버전 실빌드로 재확인.
